 ...
 */
export function generateDynamicYTT(project: Project): string {
  /* ------------------------------------------------------------------ */
  /* 0. 유틸 함수 정의                                                  */
  /* ------------------------------------------------------------------ */
  ...
  const normalizeOpacity = (value?: number): number | undefined => {
    ...
    // 0~1 사이면 0~255 로 변환, 그 외 그대로(정수 가정)
    let v = value <= 1 ? Math.round(value * 255) : Math.round(value);
    // 255 는 업로드 시 속성 유실되므로 254 로 치환
    if (v === 255) v = 254;
    return v;
  };
