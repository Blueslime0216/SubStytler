...
export const VideoController: React.FC<VideoControllerProps> = ({
  ...
  // 🎯 정확한 시간 계산 - 패딩과 트랙 너비 정확히 고려
  const getTimeFromPosition = (clientX: number): number => {
    if (!progressBarRef.current) return currentTime;
    
    const rect = progressBarRef.current.getBoundingClientRect();
    const padding = 20; // CSS 패딩과 정확히 일치
    const trackWidth = rect.width - (padding * 2); // 실제 트랙 너비
    const relativeX = clientX - rect.left - padding; // 패딩 제외한 상대 위치
    const clampedX = Math.max(0, Math.min(relativeX, trackWidth)); // 트랙 범위 내로 제한
    const percentage = trackWidth > 0 ? clampedX / trackWidth : 0;
  ...
  const getProgressFillWidth = () => {
    ...
    const padding = 20;
    const trackWidth = rect.width - (padding * 2);
    // 🔥 핵심: 진행 바 너비를 퍼센티지에 정확히 비례하여 계산
    const fillWidth = (progressPercentage / 100) * trackWidth;
    return `${fillWidth}px`;
  };

  const getVolumeFillWidth = () => {
    if (!volumeBarRef.current) return '0px';
    
    const rect = volumeBarRef.current.getBoundingClientRect();
    const padding = 8;
    const trackWidth = rect.width - (padding * 2);
    // 🔥 핵심: 볼륨 바 너비를 퍼센티지에 정확히 비례하여 계산
    const fillWidth = (volumePercentage / 100) * trackWidth;
    return `${fillWidth}px`;
  };
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* ✨ 볼륨 진행 바 - 픽셀 단위 정확한 너비 */}
                <div 
                  className="neu-volume-fill"
                  style={{ width: getVolumeFillWidth() }}
                />
